...
const TEXT_REPLACEMENTS: [(&str, &str); 1] = [("Here is some useful information about the environment you are running in:", "Environment context you are running in:")];
...
fn is_anthropic_messages_payload(value: &serde_json::Value) -> bool {
    value.get("model").is_some_and(serde_json::Value::is_string)
        && value.get("messages").is_some_and(serde_json::Value::is_array)
        && value.get("stream").is_some_and(serde_json::Value::is_boolean)
}
...
fn prepend_claude_code_system_value(value: &mut serde_json::Value) {
    ...
    match obj.get_mut("system") {
        ...
        Some(serde_json::Value::String(text)) => {
            let existing = std::mem::take(text);
            obj.insert("system".to_string(), serde_json::json!([claude_block, { "type": "text", "text": existing }]));
        }
        _ => { obj.insert("system".to_string(), serde_json::json!([claude_block])); }
    }
}
...
fn prepend_billing_header(value: &mut serde_json::Value) {
    let Some(messages) = value.get("messages").and_then(serde_json::Value::as_array) else { return; };
    let Some(first_user_text) = first_user_text(messages) else { return; };
    ...
    let Some(items) = blocks.as_array_mut() else { return; };
    if items.iter().any(|b| b.get("text").and_then(serde_json::Value::as_str).is_some_and(|t| t.contains("x-anthropic-billing-header:"))) {
        return;
    }
...
fn first_user_text(messages: &[serde_json::Value]) -> Option<&str> {
    messages.iter().find(|m| m.get("role").and_then(serde_json::Value::as_str) == Some("user"))
        .and_then(|m| match m.get("content") {
            Some(serde_json::Value::String(s)) => Some(s.as_str()),
            Some(serde_json::Value::Array(blocks)) => blocks.iter().find(|b| b.get("type").and_then(serde_json::Value::as_str) == Some("text")).and_then(|b| b.get("text")).and_then(serde_json::Value::as_str),
            _ => None,
        })
        .filter(|s| !s.is_empty())
}
...
fn build_billing_header(message_text: &str) -> String {
    ...
    let sampled: String = BILLING_HEADER_POSITIONS
        .iter()
        .map(|i| utf16.get(*i).copied().map(utf16_code_unit_to_js_char).unwrap_or('0'))
        .collect();
    ...
    format!("x-anthropic-billing-header: cc_version={CLAUDE_CODE_VERSION}.{suffix}; cc_entrypoint=sdk-cli; cch={cch};")
}
...
fn shape_system_blocks(value: &mut serde_json::Value) {
    let Some(items) = value.get_mut("system").and_then(serde_json::Value::as_array_mut) else { return; };
    for block in items {
...
fn split_assistant_tool_use_trailing_content(messages: &mut Vec<serde_json::Value>) {
    ...
    for message in messages.drain(..) {
        ...
        let Some(first_tool_idx) = content.iter().position(|b| b.get("type").and_then(serde_json::Value::as_str) == Some("tool_use")) else {
            out.push(message);
            continue;
        };
        if !content[first_tool_idx..].iter().any(|b| b.get("type").and_then(serde_json::Value::as_str) != Some("tool_use")) {
            out.push(message);
            continue;
        }
        ...
        for block in content {
            if block.get("type").and_then(serde_json::Value::as_str) == Some("tool_use") { tools.push(block.clone()); } else { non_tools.push(block.clone()); }
        }
        let mut first = message.clone();
        first.as_object_mut().unwrap().insert("content".to_string(), serde_json::Value::Array(non_tools));
        let mut second = message;
        second.as_object_mut().unwrap().insert("content".to_string(), serde_json::Value::Array(tools));
        out.push(first);
        out.push(second);
    }
    *messages = out;
...
mod tests {
    ...
    fn oauth_shaper_preserves_full_system_prompt() {
        ...
        assert!(system[0]["text"].as_str().unwrap().contains("x-anthropic-billing-header:"));
    ...
    fn oauth_shaper_adds_billing_and_classifier_rewrite() {
        ...
        let value: serde_json::Value = serde_json::from_slice(&shape_oauth_messages_payload(body)).unwrap();
        let system = value["system"].as_array().unwrap();
        // Billing header must be first; Claude Code identity second.
        assert!(system[0]["text"].as_str().unwrap_or("").contains("x-anthropic-billing-header: cc_version=2.1.169."));
        assert_eq!(system[1]["text"], CLAUDE_CODE_SYSTEM_PROMPT);
        assert!(system.iter().any(|block| block["text"].as_str().unwrap_or("").contains("Environment context you are running in:")));
    }
    ...
    fn billing_header_samples_utf16_code_units_like_reference() {
        ...
        let suffix = hex_sha256(format!("{BILLING_HEADER_SALT}\u{fffd}g0{CLAUDE_CODE_VERSION}").as_bytes())[0..3].to_string();
        assert_eq!(header, format!("x-anthropic-billing-header: cc_version={CLAUDE_CODE_VERSION}.{suffix}; cc_entrypoint=sdk-cli; cch={cch};"));
    }

    #[test]
    fn non_oauth_messages_payload_passes_through_unchanged() {
        let client = AnthropicHttpClient::new("sk-ant-api03-test".to_string());
        let body = Bytes::from(r#"{"model":"claude","stream":true,"system":[{"type":"text","text":"Real prompt"}],"messages":[{"role":"user","content":"hello"}]}"#);
